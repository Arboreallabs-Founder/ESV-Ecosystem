...
export const WIKI: Record<string, WikiSection> = {
  ...
  investors: {
    ...
    items: [
      { heading: 'Getting logos in bulk', body: 'Founders and admins get a "Get N fund logos" button in the header. It derives each logo from the domain already on the fund record, so nothing needs pasting. It works through them in batches and you can stop at any point — everything already fetched is kept.' },
      { heading: 'Why some funds keep their initials', body: 'About one in five publishes only a 16 or 32 pixel icon. Stretched to the size a card draws, that looks worse than the initials shown in its place, so anything under 64 pixels is skipped rather than made to look broken. The run says how many. Paste a logo address on the fund itself for those.' },
      { heading: 'Merging duplicates', body: 'Founders and admins get "Find duplicates" in the header. It groups records whose names match once fund suffixes are stripped, so "Blume" and "Blume Ventures" show up together. Pick which record to keep and merge — everything pointing at the other one (deals, lists, contacts, outreach, referrals) moves across, anything the keeper was missing is filled in from it, and their sector tags are combined.' },
      { heading: 'Why duplicates matter', body: 'Two records for one fund each match a deal on their own tags, so the fund appears twice in the investor-list suggestions — sometimes once as a thematic match and once as sector agnostic. That is not a fault in the matching; it is two records both telling the truth. Merging them is the fix.' },
      { heading: 'Searching for a pair yourself', body: 'The automatic grouping only catches names that match once suffixes are stripped, so it cannot see a spelling difference — "SANGITA MAHADIK" and "Sangeeta Mahadik" normalise to different names. Use the search at the top of the same screen: type part of the name, tick the records that are the same person or fund, choose which to keep, merge.' },
      { heading: 'Why it is not just made fuzzy', body: 'Loosening the automatic match to allow small spelling differences turns up 43 pairs across the book, of which about two are real. "8X Ventures" and "CS Ventures", "Artha Ventures" and "Asha Ventures", "HARSH SHAH" and "Naresh Shah" are all within the same distance as the genuine ones. A list that is 95% wrong is one nobody reads, so the judgement stays with a person and the search is there to help them make it.' },
      { heading: 'The matching is loose on purpose', body: 'It strips words like Capital, Ventures and India, so it will occasionally group two funds that are genuinely different. That is why nothing merges automatically — you choose the keeper and confirm. A merge cannot be undone.' },
  ...
  investorLists: {
    ...
    items: [
      ...
      { heading: 'Funds they want added', body: 'The other direction, on the same page: a founder can name funds they would like approached that are not on the list. You match each name to a fund we hold, and matching adds it to the list straight away. If we do not hold it, add it to the investor database first and it appears here to match.' },
      { heading: 'Editing after it has gone out', body: 'You can add or remove funds while the list is live — no need to unshare first, which used to take the founder\'s page offline mid-review. Anything added after they replied is marked "Added since you last looked" on their page, and they are asked to send again to confirm it. A closed list still has to be reopened.' },
      { heading: 'They can change their answer', body: 'The founder\'s link stays open and the thank-you screen has a way back to the form. A second submission replaces the first outright rather than merging, so there is never a half-old answer on file.' },
