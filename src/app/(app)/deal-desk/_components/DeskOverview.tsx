...
export default function DeskOverviewPanel({ overview }: { overview: DeskOverview }) {
  ...
  const kpis = [
    ...
    { label: 'Awaiting review', value: overview.unseen, foot: <span className={panels.kpiFoot}>Nobody has opened these</span> },
    { label: 'Open', value: overview.open, foot: <span className={panels.kpiFoot}>No decision recorded</span> },
    { label: 'To discuss', value: overview.discuss, foot: <span className={panels.kpiFoot}>Flagged for a conversation</span> },
    { label: 'Total ask', value: formatInrShort(overview.totalAskInr), foot: <span className={panels.kpiFoot}>Across every card on the desk</span> },
  ]
    ...
    <div className={panels.overview}>
      ...
      <section className={panels.panel}>
        <div className={panels.panelHead}>
          <h2 className={panels.panelTitle}>Deal overview</h2>
          <span className={panels.panelNote}>Last 30 days</span>
        </div>

        <div className={panels.kpiStrip}>
          {kpis.map((k) => (
            <div key={k.label} className={panels.kpi}>
      ...
      <div className={panels.overviewSide}>
        ...
        <section className={panels.panel}>
          <div className={panels.panelHead}>
            <h2 className={panels.panelTitle}>Deals by stage</h2>
        ...
        <section className={panels.panel}>
          <div className={panels.panelHead}>
            <h2 className={panels.panelTitle}>Waiting on you</h2>
            <span className={panels.panelNote}>{overview.unseen} unopened</span>
          </div>
          {overview.waiting.length === 0 ? (
            <div className={panels.chartEmpty}>Everything on the desk has been looked at.</div>
          ) : (
      ...
      <section className={panels.panel}>
        <div className={panels.tableHead}>
          <h2 className={panels.panelTitle}>All deals</h2>
          <div className={panels.tableControls}>
            ...
            />
            <select className={panels.tableSelect} value={stage ?? ''} onChange={(e) => { setStage(e.target.value || null); setPage(0) }} aria-label="Filter by stage">
              <option value="">All stages</option>
              {overview.byStage.filter((s) => s.count > 0).map((s) => (
                <option key={s.label} value={s.label}>{s.label}</option>
              ))}
            </select>
            <select className={panels.tableSelect} value={sector} onChange={(e) => { setSector(e.target.value); setPage(0) }} aria-label="Filter by sector">
              <option value="">All sectors</option>
              {sectors.map((s) => <option key={s} value={s}>{s}</option>)}
            </select>
            <select className={panels.tableSelect} value={owner} onChange={(e) => { setOwner(e.target.value); setPage(0) }} aria-label="Filter by owner">
              <option value="">All owners</option>
              {owners.map(([id, name]) => <option key={id} value={id}>{name}</option>)}
            </select>
            <select className={panels.tableSelect} value={status} onChange={(e) => { setStatus(e.target.value); setPage(0) }} aria-label="Filter by status">
              <option value="">All statuses</option>
              {Object.entries(DESK_DEAL_STATUS_LABELS).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
            </select>
            {filtersOn && <button className={panels.tableReset} onClick={reset}>Reset</button>}
          </div>
        </div>
              ...
              <table className={panels.overviewTable}>
                <thead>
                  <tr>
                    ...
                    <th className={panels.numCol}>Ask</th><th>Status</th><th>Last updated</th>
                  </tr>
                </thead>
                <tbody>
                  {visible.map((r) => (
                    <tr key={r.id}>
                      <td>
                        <Link href={`/deal-desk/${r.associate_id}`} className={panels.tableCompany}>
                          {r.company_name}
                        </Link>
                        {!r.seen_status && <span className={panels.newDot} title="Not yet reviewed" />}
                      </td>
                      <td className={panels.muted}>{r.sector ?? '—'}</td>
                      <td>{r.stage ? <span className={panels.stagePill}>{r.stage}</span> : <span className={panels.muted}>—</span>}</td>
                      <td>
                      ...
                      <td className={panels.numCol}>{r.ask_inr ? formatInrShort(r.ask_inr) : <span className={panels.muted}>—</span>}</td>
                      <td><StatusPill status={r.deal_status} /></td>
                      <td className={panels.muted}>{formatDate(r.updated_at)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
function StatusPill({ status }: { status: DeskOverviewRow['deal_status'] }) {
  ...
  const cls =
    status === 'rejected' ? panels.pillRejected
    : status === 'discuss' ? panels.pillDiscuss
    : status === 'more_info' ? panels.pillMoreInfo
    : panels.pillOpen
  return <span className={`${panels.statusPill} ${cls}`}>{DESK_DEAL_STATUS_LABELS[status]}</span>
}
